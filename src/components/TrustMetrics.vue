<template>
  <section ref="sectionRef" class="relative overflow-hidden border-y border-gray-border/30 bg-black-alt py-20 md:py-28">
    <div class="absolute inset-0 opacity-[0.03]" style="background-image: radial-gradient(circle at 50% 50%, rgba(212,175,55,0.3) 0%, transparent 50%)" />
    <div class="mx-auto max-w-7xl px-6 lg:px-12">
      <div ref="metricsGrid" class="grid grid-cols-2 gap-8 md:grid-cols-4 md:gap-12">
        ...
        >
          <p class="font-display text-4xl font-bold leading-none text-gold md:text-5xl lg:text-6xl">
            {{ Math.round(counters[i].current.value) }}{{ metric.suffix }}
          </p>
          <div class="mx-auto mt-3 h-[1px] w-12 bg-gradient-to-r from-transparent via-gold/50 to-transparent" />
